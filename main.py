...
# [1] السيرفر لضمان العمل 24 ساعة (Live)
app = Flask(__name__)
@app.route('/')
...
async def control_panel(event):
    ...
    if text == ".ايدي":
        ...
        # جلب صورة البروفايل وإرسالها مع الآيدي
        ...
        await event.edit(f"**🛡️ درع المتمرد نشط.. النظام مستقر.**") #
...
# --- [5] محرك أوامر الجروبات (Plugins) ---
def load_plugins():
    path = "plugins/*.py"
    for name in glob.glob(path):
        shortname = Path(name).stem
        try:
            # هنا يتم ربط ملفات مثل rebel_tech و tools تلقائياً
...
async def start_rebel():
    await client.start()
    # تحميل الإضافات بعد الـ start يحل مشكلة "client is not defined"
    load_plugins()
